...
const int range=4; int threshold=3; int states=3; int moore=4;
...
void main( void ) {
    ...
    //Von Neumann neighborhood
    if (moore==4) {
        for (float y=-float(range); y<=0.; y+=2.1239) {
            //int looprange = range-abs(y);
            //for (int x=-looprange; x<=looprange; x++) {
            for (float x=-float(range); x<=float(range); x+=2.4672) {
				int looprange = int(int(range)-int(abs(float(y))));
				if (int(x)<-looprange) { continue; }
				if (int(x)>looprange) { continue; }
                if (int(x)==0 && int(y)==0) {
                    //skip current cell
                } else {
